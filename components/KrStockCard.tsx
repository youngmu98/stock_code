...
import { KrStockDialog } from './KrStockDialog'
...
export function KrStockCard({ ticker }: Props) {
  const { stocks, analyzing } = useKrStocks()
    ...
    <>
      ...
      >
        <div className="flex items-start justify-between mb-3">
          <div>
            <p className="text-xs text-zinc-400 font-mono">{ticker}</p>
        ...
        <div className="mb-3">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xs font-mono text-zinc-400 font-semibold">{stock.score}%</span>
          </div>
          <div className="h-1.5 bg-zinc-800 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all ${
                stock.signal === 'BUY' ? 'bg-blue-500' : stock.signal === 'SELL' ? 'bg-red-500' : 'bg-zinc-500'
              }`}
        ...
        <div className="flex items-center justify-between">
          <span className="text-lg font-mono font-semibold text-zinc-100">
            {formatKrw(stock.currentPrice)}원
          </span>
          <div className="flex items-center gap-2">
            {isAnalyzing && <span className="w-1.5 h-1.5 rounded-full bg-zinc-500 animate-pulse" />}
            <span className={`text-sm font-mono ${stock.changePercent >= 0 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <KrStockDialog ticker={ticker} open={open} onOpenChange={setOpen} />
    </>
  )
}
